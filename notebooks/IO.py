...
class IO(object):
    ...
    def runPSTH(self, N=300, G=3, S=50, it=8, binsize=1000, d1=10000, d2=60000,
                d3=25000, before=10000, after=85000, s=60, WII=1400, LTP = 1, LTD = 1, model = 'cc-izh'):
        results = Parallel(n_jobs=num_cores)(delayed(self.runSimulation)(N,i,G,S, d1, d2, d3, before, after, s, WII, LTP, LTD, model) for i in range(it))

    def readPSTH(self, N, G, S, it, binsize, d1, d2, d3, before, after, s, WII, LTP, LTD, model, coeff):
        gr = GRAPH()
        listSSP1 = Parallel(n_jobs=num_cores)(delayed(self.readSimulationSSP1)(N,i,G,S, d1, d2, d3, before, after, WII, LTP, LTD, model) for i in range(it))
        listS = Parallel(n_jobs=num_cores)(delayed(gr.readoutSpikes)(ssp1, coeff) for ssp1 in listSSP1)

        tot = np.sum(listS, axis=0)
        total = np.sum(listS)
        h = self.getHist(tot,binsize)
        spikes_x, spikes_y, spikes_x_tc, spikes_y_tc, gamma, correlation, ssp1, stimulation,p,q,lowsp,vm = self.readSimulation(N,0,G,S, d1, d2, d3, before, after, WII, LTP, LTD, model)
    ...
    def readSimulationSSP1(self, N, i, G,S, d1, d2, d3, before, after, WII, LTP, LTD, model):
        T = d1+d2+d3
...
class GRAPH:
    ...
    def plotPTSH(self, before, after,binsize, h, s, it, DIRECTORY, S, N):
        T = before+after
        simsize=T/0.25
    ...
    def plotRaster(self, spikes_x,spikes_y):
    ...
    def fixpoint(self, df, nuEI, ratio, rule, g0 = 7):
        '''
    ...
    #     sg = 1 - np.array(bg)
        ...
        if (dg2 > 0).all() or (dg2 < 0).all():
            ...
            minval , idx =  self.find_nearest(dg2,0)
            res = sg[idx,1]
        return res
